...

      {
        data.map((item) => (
          <div className="flex flex-col mt-8 justify-center">
            ...
            {/* Other Pics Section */}
            {item.OtherPics && item.OtherPics.length > 0 && (
  <Fade bottom delay={300 * 2} triggerOnce>
    <div className="flex flex-col mt-10 mb-12 mx-8 sm:mx-16 xl:mx-28">
      ...
      <div className="flex flex-wrap justify-center">
        {item.OtherPics.map((pic) => (
          // Use a unique key here, ideally pic itself or some other unique identifier
          <div key={pic} className="m-2">
            <img src={pic} alt="Other Pic" className="rounded-xl shadow-xl w-64 h-64 object-cover" />
          </div>
        ))}
      </div>
    </div>
        ...
        <Fade bottom delay={300 * 1} triggerOnce>
              <div className="flex flex-col mt-16 mb-12 mx-8 sm:mx-16 xl:mx-28">
                ...
                <p className="font-light text-lg text-gray-400 text-justify">
                  {item.details || "Default text if no details provided."}
                </p>
              </div>
